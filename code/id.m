function [curve p_zx p_z p_yz] = id(p_x,p_yx,k,b0,b1,dT)
% Implements the iterative update for the information distortion
% Solves for p(z|x), p(z), p(y|z) given p(x) and p(y|x) 
% with k codewords and inverse temperature b
% From Dimitrov and Miller, 2001
% David Pfau, 2011
...
while 1
    ...
    Iyz = I(p_z,p_yz);
    Hzx = entropy(p_zx)*p_x;
    if abs(obj - (Hzx + b*Iyz)) < 1e-8
        if b <= b1
            break; 
        else
            curve = [curve; b, Iyz/Ixy, Hzx/log(k)];
            fprintf('Beta: %f -> %f\n',b,1/((1/b) + dT));
            b = 1/((1/b) + dT);
